...
const TechResourceTable: React.FC<RouteComponentProps> = () => {
  const context = React.useContext(ResourceContext)

  const [filterText, setFilterText] = React.useState<string>('')

  const filterResources = () =>
    context.techResources.filter(resource => !filterText || resource.title.toLowerCase().includes(filterText))

  return (
    <div>
      <TableHeader>
        <ResourceHeading>Tech Curriculum Resources</ResourceHeading>
        <CreateButton as={Link} to="new">
          + New Resource
        </CreateButton>
      </TableHeader>
      <CustomInputGroup color="white">
        <label>Filter Tech Resources</label>
...
export default TechResourceTable
