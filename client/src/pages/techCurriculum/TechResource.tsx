...
import Lessons from '../resources/resource/lessons/Lessons'
import { ResourceContext } from './TechCurriculum'
import ResourceForm from './TechResourceForm'
...
const EducatorResource: React.FC<IProps> = ({ _id = '', handleError }) => {
  ...
  const handleDeleteClicked = () => {
    if (resource && timesDeleteClicked === 1) {
      api.resources
        ...
        .then(() => {
          resourceContext.updateTechResources({ _id: resource._id, action: 'delete' })
          navigate('/tech-curriculum')
        })
        .catch(handleError)
    } else {
  ...
  return (
    <div>
      <FormHeading _id={_id} action={action} singular="Resource" plural="Resources" route="/tech-curriculum" />
      <ResourceForm
        action={action}
        handleError={handleError}
        resource={resource}
        updateResources={resourceContext.updateTechResources}
      />
      <Buttons>
        {action === 'update' &&
